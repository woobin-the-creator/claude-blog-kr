...
**`window.CBK_AT_ROOT` + `window.CBK_SITE_BASE` (set before any script tag):** `nav.js` hardcodes links relative to `posts/` — `"../index.html"` for the brand/home/breadcrumb links, `"../library.html"` twice, `"assets/cbk.css"` for its stylesheet, and a bare `p.file` for each sidebar entry. `post.html` and `404.html` are served from the repo root, so without these flags every one of those links points outside the site. Task 4 Step 3 makes `nav.js` read them (`SITE`, `BASE`, `ASSETS`, `hrefFor()`); this task's only job is to set them. If Task 4 was implemented without those helpers, go back and add them before finishing this task.

The two pages set them **differently, and this is the load-bearing part**:

| | `CBK_AT_ROOT` | `CBK_SITE_BASE` | `CBK_ASSET_BASE` |
|---|---|---|---|
| `post.html` | `true` | *unset* | `"posts/"` |
| `404.html` | `true` | `"/claude-blog-kr/"` | `"/claude-blog-kr/posts/"` |

`post.html` really is served at `/claude-blog-kr/post.html`, so plain relative links already resolve; hardcoding a base there would break opening the file locally. `404.html` is different: GitHub Pages returns it for a missing path **with the address bar unchanged**, so it executes at `/claude-blog-kr/posts/<slug>.html`. Leaving `CBK_SITE_BASE` unset there makes `index.html` resolve to `/claude-blog-kr/posts/index.html` — itself a missing path, so Pages serves `404.html` again, `slugFromLocation()` extracts the slug `index`, and the visitor reads "글을 찾을 수 없습니다: index" with no way out of the site. The sidebar has the same trap in a form that *looks* like it works: `post.html?slug=x` resolves to `/claude-blog-kr/posts/post.html?slug=x`, which renders only because the 404 fallback reads the query string first — the URL is bogus and the HTTP status stays 404. `tests/nav.test.js`'s `at404` case pins all six link kinds; a manual click-through of the sidebar on a real 404 URL is the real check.
...
Same shape as `post.html`, with three deliberate differences — `render-post.js` already recovers the slug from a `/posts/<slug>.html` path, so no separate rendering logic is needed:

1. `window.CBK_SITE_BASE = "/claude-blog-kr/"` (see the table above).
2. Absolute script paths, for the same reason.
3. A static `← 메인으로` link inside `#post-error`. Without it a visitor who lands here with a dead slug, or while Supabase is down, has no way home — and the `#post-error a` rule would style a link that never exists. `render-post.js` writes its message into `#post-error-msg` rather than over `#post-error` precisely so this link survives.
...
<p id="post-error" hidden><span id="post-error-msg"></span> <a href="/claude-blog-kr/">← 메인으로</a></p>
<div id="post-body"></div>
<!-- CBK_SITE_BASE: 이 파일은 주소창이 /claude-blog-kr/posts/<slug>.html 인 채로 실행된다.
     그래서 링크 기준을 상대경로가 아니라 Pages 베이스로 못박아야 사이트 밖으로 안 나간다. -->
<script>window.CBK_AT_ROOT = true; window.CBK_SITE_BASE = "/claude-blog-kr/"; window.CBK_ASSET_BASE = "/claude-blog-kr/posts/";</script>
